...
class WeatherDataStore(
    ...
    suspend fun getCachedWeatherState(): Result<WeatherState?> = try {
        val preferences = context.weatherDataStore.data.map { prefs ->
            val jsonStr = prefs[WEATHER_STATE_KEY]
            if (jsonStr != null) {
                json.decodeFromString(WeatherState.serializer(), jsonStr)
            } else {
                null
        ...
        var result: WeatherState? = null
        preferences.collect { state ->
            result = state
        }
    ...
    suspend fun isCacheValid(): Boolean = try {
        val preferences = context.weatherDataStore.data
        var isValid = false
        
        preferences.collect { prefs ->
            val lastUpdatedStr = prefs[LAST_UPDATED_KEY]
            if (lastUpdatedStr != null) {
                val lastUpdated = lastUpdatedStr.toLongOrNull() ?: 0L
                val ageMs = System.currentTimeMillis() - lastUpdated
                isValid = ageMs < WeatherApiConfig.CACHE_EXPIRATION_MS && 
